...
`Entity.Sparql` ADR-0002 introduced `ISparqlQueryStore` as the opt-in seam that lets a
...
A new test class `GraphDbSparqlQueryTests` in `Entity.Repository.GraphDb.Tests` mirrors
...
- `Entity.Repository.GraphDb.Tests` gains a `ProjectReference` to `Entity.Sparql` to
