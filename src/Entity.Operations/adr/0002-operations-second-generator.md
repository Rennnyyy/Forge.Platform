...
1. **Extend `Forge.Entity.Generators` to also emit Operations methods.**
   ...
   awareness of `Forge.Entity.Operations` (a runtime library that lives in a different slice).
...
2. **New generator project `src/Entity.Operations.Generators/`**, targeting `netstandard2.0`,
...
- Project: `src/Entity.Operations.Generators/Forge.Entity.Operations.Generators.csproj`
...
- The generator depends only on `Microsoft.CodeAnalysis.CSharp` + `Microsoft.CodeAnalysis.Analyzers`;
  it does **not** reference `Forge.Entity` or `Forge.Entity.Operations` at build time —
  the references are in the user's compilation, not the generator binary.
- Consumer projects must reference:
  - `Forge.Entity.Operations` (runtime library)
  - `Forge.Entity.Operations.Generators` as `OutputItemType="Analyzer" ReferenceOutputAssembly="false"`
...
- The Operations generator can evolve independently: adding a new method requires only
  touching `Entity.Operations.Generators`, not `Forge.Entity.Generators`.
