...
- New slice: `src/Entity.Operations/` → `Forge.Entity.Operations`.
- New generator slice: `src/Entity.Operations.Generators/` → `Forge.Entity.Operations.Generators`
  (see ADR-0002).
...
- `EntityOperations` exposes typed delegation helpers (`CreateAsync<T>`, `UpdateAsync<T>`,
  ...
  the emitted code free of references to `Forge.Entity.Repository` types directly.
- The slice depends on `Forge.Entity.Repository` for `IEntityStore` and `WriteMode`.
