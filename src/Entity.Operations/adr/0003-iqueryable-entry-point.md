...
entry-point for entity persistence. The newly introduced `Forge.Entity.Sparql` slice
...
1. **Add `Query<T>()` directly on `EntityOperations`**, with `Forge.Entity.Operations`
   referencing `Forge.Entity.Sparql`. The method composes with the existing CRUD
...
2. Ship `Query<T>()` as an extension method in the Sparql slice on `EntityOperations`.
   ...
   require a `using Forge.Entity.Sparql;`, which breaks the "one ambient type, all
...
- `Forge.Entity.Operations.csproj` adds a `ProjectReference` to
  `Forge.Entity.Sparql.csproj`.
