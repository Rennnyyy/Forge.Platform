...
Root ADR-0005 anticipated a future `Forge.Entity.Sparql` slice for query construction.
`Forge.Entity.Repository` ships only a coarse type-stream API (`QueryByTypeAsync<T>`) plus
...
1. **New slice `Forge.Entity.Sparql`** that ships a typed `IQueryable<T>` provider and a
