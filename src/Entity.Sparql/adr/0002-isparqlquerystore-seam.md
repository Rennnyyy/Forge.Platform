...
1. **Place `ISparqlQueryStore` in `Forge.Entity.Repository`** as a sibling interface to
...
2. Place `ISparqlQueryStore` in `Forge.Entity.Sparql` and have each back-end take a
...
`RdfTerm` (already defined in `Forge.Entity.Repository.Rdf`) keeps the shape uniform
...
`Forge.Entity.Sparql.EntitySparqlExtensions.Query<T>()` requires the ambient or passed
...
- The Sparql slice depends only on `Forge.Entity.Repository`, not on any specific
  back-end.
