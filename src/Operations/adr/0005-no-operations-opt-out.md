...
`Forge.Operations.Generators` (Operations ADR-0002) emits active-record CRUD
...
1. **Add a `[NoOperations]` attribute in `Forge.Operations`. The Operations
   generator skips emission when the symbol carries it.** The attribute lives in the
   Operations slice; consumers reference it the same way they reference `Forge.Operations`.
...
`Forge.Operations.Generators` reads `NoOperationsAttribute` by symbol name from
...
- The `[NoOperations]` attribute lives in `Forge.Operations`, so a consumer
